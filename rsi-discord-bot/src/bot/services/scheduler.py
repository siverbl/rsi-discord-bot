...
class RSIScheduler:
    ...
    async def _run_autoscan(self, region: str):
        ...
        # Calculate batch count (approximate based on provider)
        from bot.config import TV_BATCH_SIZE, RSI_PROVIDER
        if RSI_PROVIDER == 'tradingview':
            batch_count = (len(region_tickers) + TV_BATCH_SIZE - 1) // TV_BATCH_SIZE
        else:
            from bot.config import BATCH_SIZE
            batch_count = (len(region_tickers) + BATCH_SIZE - 1) // BATCH_SIZE
